...
// DONT take out the 'tmp'. Turns out there is a weird bug in GCC in which
...
struct LiChao_dyn{
	...
	vector<Node> pool;int pool_sz; inline void reset()noexcept{pool_sz=0;}
	constexpr LiChao_dyn(int preres=0){reset();pool.reserve(preres);}
	inline int new_tree(){if(int(pool.size())==pool_sz)pool.push_back(Node());
	...
	int update(int nd, Point<Tpt>ln, lint l=tl,lint r=tr){
		...
		if     (eval(ln,l)<eval(aln,l)){
			const int tmp=update(pool[nd].chld[0],ln, l ,m);
			pool[nd].chld[0]=tmp;
		} else if(eval(ln,r)<eval(aln,r)){
			const int tmp=update(pool[nd].chld[1],ln,m+1,r);
			pool[nd].chld[1]=tmp;
		} return nd;
