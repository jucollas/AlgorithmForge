...
void build_lca(const vector<vector<int>>&t,const vector<Data> &arr,int nd=0,int p=0,int d=0){
	...
	while(u!=v){ int jmp=pi[u][1]!=pi[v][1];
		l=l+bl[u][jmp];u=pi[u][jmp];
		r=r+bl[v][jmp];v=pi[v][jmp];//notice bl[v][?] doesnt include pi[v][?]
	} return l+bl[u][0]+r.invert();//notice I am already on u
    ...
    const int p=lca(u,v); // counts distance in edges
    return dpt[u]+dpt[v]-2*dpt[p];//+1 to count in nodes
}
