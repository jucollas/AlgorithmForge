...
template<typename T> void trans_subset(vector<T> &vec,int sd){
	// I assume $sd\in\{-1,1\}$
...
} template<typename T> vector<T> or_conv(vector<T> A,vector<T> B){
	...
	trans_subset(A,-1);
	return A;
} template<typename T> void trans_superset(vector<T> &vec,int sd){
	// I assume $sd\in\{-1,1\}$
...
} template<typename T> vector<T> and_conv(vector<T> &A,vector<T> &B){
	// the answer is shown in A; I assume $|A|=|B|=2^x$ for some x
	// Computes A'[k]=\sum_{(i&j)==k}A[i]*B[j]$
	...
	trans_superset(A,-1);
	return A;
} template<typename T> vector<T> subset_conv(const vector<T> &A,const vector<T>&B){
	// the answer is shown in A; I assume $|A|=|B|=2^x$ for some x
	...
	vector<T>B(1<<n); rep(k,0,n+1){
		...
		trans_subset(B_hat[k],-1); rep(j,0,1<<n){
