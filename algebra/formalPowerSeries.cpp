...
}template<typename tfps,bool inv=bool(0),int n=min<uint64_t>(1<<30,(tfps::mod()-1)&(1-tfps::mod()))>
...
} template<typename tfps,int N=min<uint64_t>(1<<30,(tfps::mod()-1)&(1-tfps::mod()))>
...
template<typename tfps> struct FormalPowerSeries{ std::vector<tfps> F;
    ...
    } template<int N=std::min<uint64_t>(1<<30,(tfps::mod()-1)&(1-tfps::mod()))>
