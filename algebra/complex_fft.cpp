...
Tested/develped in: https://qoj.ac/contest/1814/problem/300
*/ typedef complex<double> cmplx; // const double PI = acos(-1);
cmplx knroot(int n,int k){double tmp=double(2*k)*M_PI/double(n);return {cos(tmp),sin(tmp)};}
struct fft_info{ vector<cmplx> omega,iomega;
	fft_info(int lgi){
		const int n=1<<lgi;
		omega.resize(n);iomega.resize(n);
		cmplx w=knroot(n,1),iw=knroot(n,n-1);
		omega[0]=1;iomega[0]=1;
		for(int i=1;i<n;++i)
			omega[i]=omega[i-1]*w,iomega[i]=iomega[i-1]*iw;
	} fft_info()=default;
};array<fft_info,24> f_info;
void fft( vector<cmplx> &A,bool invert ){ // Coley-Tuckey
	...
	for(int m=1,acb=1;m<=lgi;++m,acb<<=1){
		int l=1<<(lgi-m); for(int s=0;s<(1<<lgi);s+=1<<m){
			int ind=0; for(int j=0;j<acb;++j){
				cmplx v=A[s+j+acb]*om[ind];
				A[s+j+acb]=A[s+j]-v; A[s+j]+=v;
