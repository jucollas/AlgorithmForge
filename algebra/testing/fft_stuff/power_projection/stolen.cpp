...
vc<mint> power_projection(vc<mint> f, vc<mint> wt, int m) {
  ...
  // FOR(i,2*n*k)cerr<< P[i].val << " ";
  // cerr<<endl;FOR(i,2*n*k)cerr<< Q[i].val << " ";
  // cerr<<endl; exit(0);

  while (n > 1) {
    // cerr<<"[P]"; FOR(i,2*n*k)cerr<< " " << P[i].val; cerr<<endl;
    // cerr<<"[Q]"; FOR(i,2*n*k)cerr<< " " << Q[i].val; cerr<<endl;


    vc<mint> R(2 * n * k);
    FOR(i, 2 * n * k) R[i] = (i % 2 == 0 ? Q[i] : -Q[i]);

    // cerr<<"[R_]"; FOR(i,2*n*k)cerr<< " " << R[i].val; cerr<<endl;

    // P(x)Q(-x),Q(x)Q(-x)
    vc<mint> PQ = convolution(P, R), QQ = convolution(Q, R);
    PQ.resize(4 * n * k), QQ.resize(4 * n * k);

    // cerr<<"[QQ]"; FOR(i,4*n*k)cerr<< " " << QQ[i].val; cerr<<endl;
    ...
    // cerr<<"[PQ]"; FOR(i,4*n*k)cerr<< " " << PQ[i].val; cerr<<endl;
...
vc<mint> compositional_inverse(vc<mint> f) {
  ...
  */
  ...
  cerr<<"[A]"; FOR(i,n+1)cerr<< " " << A[i].val; cerr<<endl;
  vc<mint> g(n);
  FOR(i, 1, n + 1) g[n - i] = mint(n) * A[i] * inv<mint>(i);
  cerr<<"[g]"; FOR(i,n)cerr<< " " << g[i].val; cerr<<endl;
...
void solve() {
  vc<mint> F={0, 1, 753573084, 722801665}; // vc<mint>({0,1,2,3})
  vc<mint> W={0, 0, 0, 1};//vc<mint>({4,3,2,1})
  F={0, 1, 398829917, 254115598,0};
  W={0, 0, 0, 0, 1};
  F={0, 48, 899930270 ,355122750 ,885751026, 105435380 ,936068561 ,694651622};
  // vc<mint> ar=power_projection(F,W,5);

  vc<mint> ar=compositional_inverse(F);
  for(auto&ac:ar)cerr << ac.val << " ";
  cerr << endl;
  return;
