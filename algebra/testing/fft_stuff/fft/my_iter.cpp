...
struct fft_info{
	...
	fft_info(){
		root[rank2]=mpow(c_root,(mint::mod-1)>>rank2,mint::mod);
		rep(i,rank2-1,-1) root[i]=root[i+1]*root[i+1];
		
		iroot[rank2]=mpow(root[rank2],mint::mod-2,mint::mod);
		rep(i,rank2-1,-1) iroot[i]=iroot[i+1]*iroot[i+1];
		
		// idebug(root);idebug(iroot);
	}
};

	/*for(int m=1;m<=lgi;++m){
		// debug(m);
		mint wlen=(invert)?info.iroot[m]:info.root[m];
		
		int acb=1<<(m-1);
		for(int s=0;s<(1<<(lgi-m));++s){
			int off=s<<m;
			mint w=1;//int cnt=0;
			for(int j=0;j<acb;++j){
				// debug(off+j,off+j+acb,nm[off+j],nm[off+j+acb],w,wlen);
				mint u=A[off+j],v=A[off+j+acb]*w;
				// debug(u+v,u-v,u,v);
				A[off+j    ]=u+v;
				A[off+j+acb]=u-v;
				w*=wlen;//++cnt;
...
void gentlemanSande(fps&A,bool invert){
	const int n=sz(A),lgi=ilog2(n);
	static const fft_info info;
