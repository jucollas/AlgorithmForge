...
I still dont understand why my impl requires a *2 in the calculation of the convolution for the 
	inner_product
*/
...
void iter_fft( vector<mint> &A,bool invert ){
	// Coley-Tuckey
...
const bool use_butterfly=1;
void fft(std::vector<mint> &A,bool invert){
	if(invert){
		if(use_butterfly) internal::butterfly_inv(A);
		else internal::iter_fft(A,invert);
		
		mint n_1 = mint(sz(A)).inv();
		for (mint & x : A)x*=n_1;
	} else if(use_butterfly) internal::butterfly(A);
	else internal::iter_fft(A,invert);
}
...
fps brute_iproduct(const fps &A,const fps &B){
	const int n=sz(A),m=sz(B);
	fps C(n,0);rep(i,0,n)rep(j,0,m){//min(m,i+1)){
		// debug(i,j,i-j,A[i],B[j]);
		if(i>=j)C[i-j]+=A[i]*B[j];
		else C[n-i+j]+=A[i]*B[j];
	} return C;
}
fps inner_product(fps A,fps B){
	const int n=sz(A);
	// int d=1;while(d<n)d*=2;
	// d*=2;
	 int L = 1;
  while (L < sz(A)) L *= 2;
  
	const int d=1<<(ilog2(n-1)+2);
	// debug(d,sz((A*B)),L);
	A.F.resize(d,0);B.F.resize(d,0);
	// idebug(B);
	
	internal::fft(B.F,0);
	internal::transposed_fft(A.F,1);
	// idebug(B);idebug(A);
	rep(i,0,d)A[i]*=B[i];
	
	internal::transposed_fft(A.F,0);
	return A.trunc(n);
}
...
void solve() {
	...
	fps C=inner_product(A,B);
	rep(i,0,n)cout << C[i] << " \n"[i+1==n];
	
	return;
	fps tc=brute_iproduct(A,B);
	rep(i,0,n)cout << tc[i] << " \n"[i+1==n];
}
