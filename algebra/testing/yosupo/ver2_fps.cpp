...
template<lint raw_m,typename tint=int,typename tmul=lint,bool arbi_ntt=0>
struct modulo_int{
    ...
    inline constexpr modulo_int pow(lint e)const{return modulo_int(mpow<tint,tmul>(vl,e>=0?e%(m-1):(((m-2)*1ll*(-(e%(m-1))))%(m-1)),m));}
...
#ifdef INV_PRECALCULATE
template<typename tint> struct Prec{
    vector<tint> fact,ifact; constexpr Prec(int n){
        fact.resize(n);ifact.resize(n);
        fact[0]=1;rep(i,1,n)fact[i]=fact[i-1]*tint(i);
        ifact[n-1]=fact[n-1].inv();
        rep(i,n-1,0)ifact[i-1]=ifact[i]*tint(i); }
    inline tint inv(int n)const{return ifact[n]*fact[n-1];}
};const Prec<mint> prec(1<<ilog2((mod-1)&(1-mod)));
#endif
...
Notes: (1ll<<59)*27+1 is a 64-bit prime (IntroMathComputational-Shoup pg 484) (overflow)
        . (549755813881ll<<24)+1 is too (https://codeforces.com/blog/entry/75326)
*/ namespace internal {
template<typename tint> constexpr int countr_zero_constexpr(tint n){int x=0;while(!(n&(tint(1)<<x)))++x;return x;}
template<typename tint> constexpr tint primitive_root(tint m){
    if( (549755813881ll<<24)+1 == m || 998244353 == m ) return 3;
    //tests for g such that $\forall_{p|(md-1)} g^{(md-1)/p}=1(mod md)$
    tint dec[32]={},dind=0, md=m-1;
    for(tint i=2;i*i<=md;++i)if(md%i==0){
        dec[dind++]=i; while(md%i==0)md/=i;
    } if(md>1)dec[dind++]=md;
    
    bool fnd=0; tint pr=1;while(!fnd){
        ++pr; fnd=1; // by properties, this will always end
        for(int i=0;i<dind&&fnd;++i)fnd=mpow<tint>(pr,(m-1)/dec[i],m)!=1;
    } return pr;// std::cerr <<pr << " _ primitive root" << endl;
} template<typename tfps,int N> constexpr tfps fft_root(){
    if constexpr(N<=0)return tfps(0);
    constexpr tfps c_root=primitive_root(tfps::mod());
    if constexpr( N>=((tfps::mod()-1)&(-tfps::mod()+1)) )return c_root.pow((tfps::mod()-1)/N);
    else return fft_root<tfps,N*2>()*fft_root<tfps,N*2>();
} template<typename tfps,int N> constexpr tfps fft_iroot(){
    if constexpr(N<=0)return tfps(0);
    if constexpr( N>=((tfps::mod()-1)&(1-tfps::mod())) )return fft_root<tfps,N>().inv();
    else return fft_iroot<tfps,N*2>()*fft_iroot<tfps,N*2>();
...
}template<typename tfps,bool inv=bool(0),int n=(tfps::mod()-1)&(1-tfps::mod())>
...
} template<typename tfps> void fft(std::vector<tfps>&a,bool invert){
...
template<typename tfps> struct FormalPowerSeries{ std::vector<tfps> F;
    ...
    FormalPowerSeries<tfps>& square(){
        ...
        #ifdef INV_PRECALCULATE
            if(size()+1<int(prec.fact.size())){
                for(int i=0;i<size();++i)G[i+1]=F[i]*prec.inv(i+1);
                return G;
            }
        #endif
        for(int i=0;i<size();++i)G[i+1]=F[i]/tfps(i+1);
        return G; // could precompute all inverses in range???
    ...
    } template<int N=((tfps::mod()-1)&(1-tfps::mod()))>
